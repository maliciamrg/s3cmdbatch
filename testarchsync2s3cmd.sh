...
(
    function quit {
        if [ -f $tmpfile ]; then
          ...
          $logger_cmd "archsync2s3cmd : backup en cours ${filestring:$numf} sur ${filestring:$numd:lend}"
        fi
    	exit 1
    }
    flock -n 200 || quit;
	...
	do 
		...
		if (($nbupl <= 0)) ;	then
			...
			if (($nbdel > $nbfillvl)) ;	then
            	...
            	$logger_cmd "archsync2s3cmd : trop de delete pour $param2 => $nbdel (nbdel) ,  $nbfillvl (threshold) "
			else
            	$logger_cmd "archsync2s3cmd : lancement backup pour $param2 => $nbupl (nbupload) , $nbdel (nbdel) "
				#s3cmd sync $param1/$param2/ -p -r -v --delete-removed s3://malicia-warehouse-$param2/ >> $tmpfile  2>&1
				$logger_cmd "${param2} done"
				$logger_cmd "archsync2s3cmd : fin backup pour $param2"
			fi;
		fi;
	done 
	exec 0<&-
	$logger_cmd "unlocked"
) 200>testarchsync2s3cmd.lock
 $logger_cmd "stop"
